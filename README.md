...
Version 2.1.9 uses separate OCR layouts for standard shooters, Mini Block Towers, ZG Marbles and escape games. It reads up to two player rows per team (four players total), recognizes the venue labels `ShowBattle`, `ZGMarbles`, `Joller` and `DGB`, and enhances the faint player-name strips on escape results before reading them. Joller and DGB use the shared `Escape Time` as each detected player's result score.
